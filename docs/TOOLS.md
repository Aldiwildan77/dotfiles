...
Each of these is a stow package whose contents mirror `$HOME`.

| Package | Links to | Notes |
| --- | --- | --- |
| `zsh` | `~/.zshrc` | modular `zshrc.d/`, split around oh-my-zsh |
| `bash` | `~/.bashrc`, `~/.bash_profile` | fallback shell on servers; same PATH as zsh |
| `vim` | `~/.vimrc` | Flexoki Dark inline, no plugin manager needed |
| `tmux` | `~/.tmux.conf` | `C-a` prefix, vi copy mode, Flexoki status line |
| `screen` | `~/.screenrc` | 256-colour, 10k scrollback, 1-indexed windows |
| `git` | `~/.gitconfig`, `~/.gitignore_global` | work identity via `includeIf` |
| `gh` | `~/.config/gh/config.yml` | `co: pr checkout` alias; token stays untracked |
| `yazi` | `~/.config/yazi/` | git status column, `Y` copies file contents |
| `zed` | `~/.config/zed/settings.json` | VSCode keymap, 2-space tabs, `ssh_connections` stripped |
| `editorconfig` | `~/.editorconfig` | 2-space default, LF, trim trailing whitespace |
| `dig` | `~/.digrc` | `+nocmd` |
| `wget` | `~/.wgetrc` | 60s timeout, 3 retries, sane defaults |

The vim, tmux and yazi palettes are the same Flexoki Dark values, so the three
...
- **vim-plug** — the old `vim/.vimrc` bootstrapped it and ran `curl` on first
  launch. The config needs no plugins, so the whole block went.
- `~/.ssh/config`, `~/.docker/config.json`, `~/.config/gcloud`,
  `~/.config/psysh/psysh_history` — credentials, private hostnames or session
  data. Per-machine by nature.
- `~/.config/{temporalio,uv,mole,configstore,github-copilot}` — install
  receipts and machine state, regenerated by the tools themselves.
